...
{    if (stream == null || feof(stream) || delimiters == null || strlen(delimiters) == 0)
    ...
    while (true)
    {
        ...
        if (next_match - cur_pos == 0)
        {
            next_match = streamposbrk(stream, ++cur_pos, delimiters);
            len = next_match - cur_pos;
            continue;
        }
...
{
    if (prev.start_pos == EOF || prev.delimiters == null || prev.length == 0 || prev.src_stream == null)
    {
        return (stream_token) { .start_pos = EOF, .length = 0, .delimiters = null, .src_stream = null };
    }
    return stream_split(prev.src_stream, prev.start_pos + prev.length, prev.delimiters);
}
